from fastapi import Request
from app.models import global_models as models
from datetime import datetime
import asyncio
from concurrent.futures import ThreadPoolExecutor

_executor = ThreadPoolExecutor(max_workers=2)

def _write_auditoria(user_id, accion, detalle, ip, path):
    """Runs in a thread pool to avoid blocking the event loop."""
    if "/tecnicos/perfil/ubicacion" in path:
        return
    from app.core.database import SessionLocal
    db = SessionLocal()
    try:
        nueva_auditoria = models.Auditoria(
            usuario_id=user_id,
            accion=accion,
            detalle=detalle,
            ip=ip,
            fecha=datetime.utcnow(),
            hora_inicio=datetime.utcnow(),
            hora_cierre=None
        )
        db.add(nueva_auditoria)

        # Cerrar sesión anterior para mantener KPIs limpios
        registro_anterior = db.query(models.Auditoria).filter(
            models.Auditoria.usuario_id == user_id,
            models.Auditoria.hora_cierre == None
        ).order_by(models.Auditoria.id.desc()).first()

        if registro_anterior:
            registro_anterior.hora_cierre = datetime.utcnow()

        db.commit()
    except Exception as e:
        print(f"!!! ERROR CRITICO AUDITORIA (User ID: {user_id}): {e}")
        import traceback
        traceback.print_exc()
    finally:
        db.close()


def registrar_auditoria(request: Request, user_id, accion, detalle):
    """Fire-and-forget: submits audit write to a thread pool so it never blocks requests."""
    if "/tecnicos/perfil/ubicacion" in request.url.path:
        return
    ip = request.client.host if request.client else "127.0.0.1"
    path = request.url.path
    _executor.submit(_write_auditoria, user_id, accion, detalle, ip, path)
